import { _userAndToken } from './../types/account.types'
...
export const MessageController = new Elysia({
    ...
    .ws('/ws', {
        ...
        async open(ws) {
            ...
            if (!payload) {
                ws.send({ sender: 'system', content: 'โง่หลายก็ตายสะ' })
                ws.close()
            }
            const user_id = (payload as AuthPayload).id
            const groupName = getGroupName(user_id, recipient_id!)

            ws.send({ sender: 'system', content: 'เก่งเหมือนกันนิหว่า:)' })
        ...
        async message(ws, message) {
            ...
            try {
                ...
                ws.send({ sender: 'system', content: 'Something wen wrong, fail to send message !!:)' })
                return
            }
        },
